...
ImGuiStyle::ImGuiStyle()
{
	...
	WindowPadding = ImVec2(5, 27);      // Padding within a window
	...
	ItemSpacing = ImVec2(20, 4);      // Horizontal and vertical spacing between widgets/lines
...
void ImGui::UpdateMouseWheel()
{
	...
	if (g.IO.MouseWheel != 0.0f)
	{
		...
		else if (!g.IO.KeyCtrl && scroll_allowed)
		{
			...
			const float scroll_speed = ImFloor(window->CalcFontSize() * 100 * g.IO.DeltaTime + 10.5f); // smooth scrolling
			SetWindowScrollY(scroll_window, scroll_window->Scroll.y - g.IO.MouseWheel * scroll_speed); // scroll_speed
		}
	}
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	if (!(flags & ImGuiWindowFlags_NoBackground)) {
		PushStyleColor(ImGuiCol_Text, ImVec4(155 / 255.f, 158 / 255.f, 255 / 255.f, 1.f));
		RenderText({ child_window->DC.CursorPos.x + 5, child_window->DC.CursorPos.y + 3 }, name);
		PopStyleColor();
	}
