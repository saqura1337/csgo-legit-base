...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect total_bb(pos, pos + ImVec2(20 + label_size.x, 14));
	ItemSize(total_bb, style.FramePadding.y);
	...
	ImGui::PushStyleColor(ImGuiCol_Text, *v ? ImVec4(1, 1, 1, 1) : ImVec4(0.7f, 0.7f, 0.7f, 1.f));
	...
	if (*v) {
		if (alpha->second < 1)
			alpha->second += GetIO().DeltaTime * 5; // bigger - faster
	}
	else {
		if (alpha->second > 0)
			alpha->second -= GetIO().DeltaTime * 5; // bigger - faster
	...
	window->DrawList->AddRectFilled(total_bb.Min, { total_bb.Min.x + 14, total_bb.Max.y }, ImColor(155 / 255.f, 158 / 255.f, 255 / 255.f, alpha->second), 2);
	if (*v)
		RenderCheckMark(window->DrawList, { total_bb.Min.x + 3, total_bb.Min.y + 3 }, ImColor(255, 255, 255), 9);
	else
		window->DrawList->AddRectFilled(total_bb.Min, { total_bb.Min.x + 14, total_bb.Max.y }, ImColor(15, 15, 15), 2);

	if (label_size.x > 0.0f)
		RenderText({ total_bb.Min.x + 20, total_bb.Min.y + 7 - label_size.y / 2 }, label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 7));
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 2), total_bb.Max);
	...
	//main
	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(12, 12, 12), 2);
	window->DrawList->AddRectFilled(frame_bb.Min, { grab_bb.Min.x, frame_bb.Max.y }, ImColor(155, 158, 255), 2);
	//

	// Render grab (circle at the end)
	window->DrawList->AddRectFilled({ grab_bb.Min.x - 1, frame_bb.Min.y - 1 }, { grab_bb.Max.x + 2, frame_bb.Max.y + 1 }, ImColor(36, 36, 36), 2);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		if (ColorButton("##ColorButton", col_v4, flags, ImVec2(20, 15))) //third arg: ImVec2(-1, -1)
		{
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
	...
	else
	{
		...
		if (selected)
			RenderCheckMark(window->DrawList, pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f), GetColorU32(enabled ? ImGuiCol_Text : ImGuiCol_TextDisabled), g.FontSize * 0.866f);
	}
...
bool ImGui::LabelClick(const char* label)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImVec2 pos = window->DC.CursorPos;
	const ImRect total_bb(pos, pos + label_size);
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, id))
	{
		IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
